@php use Devrabiul\ToastMagic\Facades\ToastMagic; @endphp
    <!DOCTYPE html>
...
<script>
    ...
    async function logout() {
        ...
        const storedToken = window.localStorage ? localStorage.getItem(tokenKey) : null;
        ...
        try {
            const response = await fetch('{{ route('logout') }}', {
                ...
                body: new URLSearchParams({ _token: csrfValue }).toString(),
            ...
            throw new Error('Unexpected response while logging out.');
        } catch (error) {
            Swal.close();
            Swal.fire('Logout failed', error.message || 'Unable to complete logout. Please try again.', 'error');
        }
    }
